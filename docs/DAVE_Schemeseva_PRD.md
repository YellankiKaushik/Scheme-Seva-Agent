...
- **Data Staleness:** Government schemes change frequently. _Mitigation:_ Prominent "Last Verified" dates and scheduled Vigilance re-checks.
- **Eligibility Ambiguity:** Some rules are subjective. _Mitigation:_ Use of "Medium Confidence" scores and directing users to official departments for final confirmation.
- **LLM Timeouts:** Multi-agent chains can be slow. _Mitigation:_ Optimized Vercel `maxDuration` and sequential loading indicators in the UI.
