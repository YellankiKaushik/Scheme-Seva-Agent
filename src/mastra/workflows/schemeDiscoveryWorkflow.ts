...
export const schemeDiscoveryWorkflow = {
  ...
  async run(input: SchemeDiscoveryInput): Promise<SchemeDiscoveryResult> {
    ...
    if (input.profile) {
      ...
      if (followUp)
        return { profile, followUp, report: null, agentSteps: { profileAgent: profileStep } };
    } else {
    ...
    const mergedSteps: DiscoveryAgentSteps = {
      ...
      discoveryAgent:
        report.agentSteps?.discoveryAgent ?? fallback("unknown", "Discovery metadata unavailable."),
      eligibilityAgent:
        report.agentSteps?.eligibilityAgent ??
        fallback("unknown", "Eligibility metadata unavailable."),
      reportAgent:
        report.agentSteps?.reportAgent ?? fallback("unknown", "Report metadata unavailable."),
      safetyValidation:
        report.agentSteps?.safetyValidation ?? fallback("unknown", "Safety metadata unavailable."),
    };
