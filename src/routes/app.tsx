...
export const Route = createFileRoute("/app")({
    head: () => ({
        meta: [
            ...
            { name: "description", content: "Describe your situation in plain English. The SchemeSeva agent finds Indian government schemes you likely qualify for in under a minute." },
            { property: "og:title", content: "Run the SchemeSeva agent" },
            { property: "og:description", content: "Free civic AI tool that discovers government welfare schemes for Indian citizens." },
        ],
    }),
    component: AgentApp,
});
...
function AgentApp() {
    ...
    const [vigilance, setVigilance] = useState<{ newMatches: number; alerts: Array<{ id: string; schemeName: string; reason: string; urgency: string; validationProvider?: string; retrievalProvider?: string; memoryProvider?: string }> } | null>(null);
    ...
    async function handleIntake(inputText: string) {
        ...
        setStage("running");
        try {
            const { profile: p, followUp: f, report: workflowReport } = await schemeDiscoveryWorkflow.run({ sessionKey, text: inputText });
            setProfile(p);
            if (f) {
                setFollowUp(f);
                setStage("clarify");
    ...
    async function runFullDiscovery(p: CitizenProfile) {
        setStage("running");
        try {
            const { report: workflowReport } = await schemeDiscoveryWorkflow.run({ sessionKey, profile: p });
                ...
                {stage === "intake" && (
                    <section>
                        ...
                        <p className="mt-2 text-muted-foreground">
                            Describe your situation in plain English — state, age, occupation, income, family.
                            The Profile Agent will extract what it needs.
                        </p>
                        <textarea
                            value={text}
                            ...
                            <div className="flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-muted-foreground">
                                        The agent scans your saved profile against schemes you haven't seen and fires an alert if you likely qualify — without being asked.
                                    </p>
                                </div>
                                <button
                                        ...
                                        vigilance.alerts.map((a) => (
                                            <div key={a.id} className="rounded-lg border border-accent/40 bg-card p-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <h4 className="font-display font-semibold text-primary">{a.schemeName}</h4>
                                                </div>
                                                <p className="mt-1 text-sm text-muted-foreground">{a.reason}</p>
                                                <p className="mt-2 text-xs text-muted-foreground">
                                                    Safety: {a.validationProvider ?? "fallback"} · Retrieval: {a.retrievalProvider ?? "session memory"} · Memory: {a.memoryProvider ?? "fallback"}
                                                </p>
                                            </div>
                                        ))
                                    )}
