...
function Pill({ ok, label }: { ok: boolean; label: string }) {
    return (
        <span
            className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${ok ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"
                }`}
        >
            {ok ? "● " : "○ "}
            {label}
                ...
                {isLoading || !data ? (
                    ...
                    <div className="mt-8 space-y-4">
                        <Card
                            ...
                            badges={[
                                <Pill key="w" ok={Boolean(data.currentWorkflowMode)} label={`mode: ${data.currentWorkflowMode}`} />,
                                <Pill key="v" ok={data.vigilanceAvailable} label={data.vigilanceAvailable ? "vigilance yes" : "vigilance no"} />,
                            ]}
                        >
                            <p className="text-sm text-muted-foreground">
                                Retrieval provider: <code>{data.currentRetrievalProvider}</code> · Safety provider:{" "}
                                <code>{data.currentSafetyProvider}</code>
                            </p>
